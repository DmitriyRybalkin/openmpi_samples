...
/* MPI custom library, Version 1
 * 
 * This library contains macros and functions for
 * parallel algorithms implemented with the help
 * of MPI technology.
 * 
...
/* expands to an expression whose value is the first, or lowest,
 * index controlled by the process
 */
#define BLOCK_LOW(id, p, n) ((id) * (n) / (p))

/* expands to an expression whose value is the last, or highest,
 * index controlled by the process
 */
#define BLOCK_HIGH(id, p, n) (BLOCK_LOW((id) + 1, p, n) - 1)

/* evaluates to the number of elements controlled by process id
 */
#define BLOCK_SIZE(id, p, n) (BLOCK_LOW((id) + 1, p, n) - BLOCK_LOW(id, p, n))

/* evaluates to the rank of the process controlling that element
 * of the array
 */
...
/*
 * Given MPI_Datatype 't', function 'get_size' returns
 * the size of a single datum of that data type
 */
...
/*
 * Function 'my_malloc' is called when a process
 * wants to allocate some space from the heap.
 * If the memory allocation fails, the process prints
 * an error message and then aborts execution of the
 * program
 */
void *my_malloc(
...
/*
 * Function 'terminate' is called when the program
 * should not continue execution, due to an error
 * condition that all of the processes are aware of.
 * Process 0 prints the error message passed as an
...
/*
 * Open a file containing a vector,
 * read its contents and replicate
 * among all processes in a
 * communicator.
 */
  ...
  MPI_Comm comm)	/* IN - Communicator */
{
    ...
    int p;		/* Number of processes */
    void *rtptr;	/* Pointer into 'storage' */
     ...
     * Allow double subscripting through 'a'
     */
    ...
    *subs = (void **)my_malloc(id, local_rows * PTR_SIZE);
     ...
     * and sends each block to the correct destination
     * process. The last block it keeps
     */
    if(id == (p - 1)) {
     for(i = 0; i < p - 1; i++)  {
 ...
 * and 'n' columns). What follows are 'm'*'n' valus
 * representing the matrix elements stored in row-major order.
 * This function allocates blocks of columns of the matrix
  ...
  MPI_Comm comm)		/* IN - Communicator */
{
     ...
     * cols, and broadcasts this into to other procs
     */
    if(id == (p - 1)) {
      infileptr = fopen(s, "r");
      if(infileptr == NULL) *m = 0;
  ...
  MPI_Comm comm)	/* IN - Communicator */
{
  ...
  if(id == (p - 1)) {
   infileptr = fopen(s, "r");
   if(infileptr == NULL) *n = 0;
   else fread(n, sizeof(int), i, infileptr);
  }
  MPI_Bcast(n, 1, MPI_INT, p - 1, comm);
  if(! *n) {
   if(!id)  {
 ...
 * decomposition among the elements of a
 * communicator
 */
  ...
  MPI_Comm comm)	/* IN - Communicator */
{
  ...
  for(i = 0; i < m; i++) {
    MPI_Gatherv(a[i], BLOCK_SIZE(id, p, n), dtype, buffer, rec_count, rec_disp, dtype, 0, MPI_COMM_WORLD);
    if(!id) {
...
/*
 * Print elements of doubly subscripted array
 */
...
/*
 * Print a matrix distributed checkboard fashion
...
/*
 * Print a matrix that is distributed
 * in row-striped fashion among the
 * processes in a communicator
 */
